...
alter table if exists player_dto drop
    constraint if exists UK_5w987jkpxq935uh3m47hrjqsa;

alter table if exists player_dto
    add constraint UK_5w987jkpxq935uh3m47hrjqsa
    unique (nickname);

alter table if exists player_dto drop
    constraint if exists UK_3ggs9a0myucan5u413kvosuc;

alter table if exists player_dto
    add constraint UK_3ggs9a0myucan5u413kvosuc
    unique (vk_id);

create sequence hibernate_sequence start 1 increment 1;

alter table if exists numbers_statistics_all
    add constraint FKpidpxqnpx4ujj9lu0fq0ogu2n
    foreign key (place_eight_id)
    references place_all;

alter table if exists numbers_statistics_all
    add constraint FKq0leo2bb7wlkc70d1j9f5wx2i
    foreign key (place_five_id)
    references place_all;

alter table if exists numbers_statistics_all
    add constraint FKmxru3yfsatkxfj7k12uhd285n
    foreign key (place_four_id)
    references place_all;

alter table if exists numbers_statistics_all
    add constraint FKtb3ofl100ba69kulg6sy80qdy
    foreign key (place_nine_id)
    references place_all;

alter table if exists numbers_statistics_all
    add constraint FKe61qp8cr5i4mc08is9mbw84rc
    foreign key (place_one_id)
    references place_all;

alter table if exists numbers_statistics_all
    add constraint FKprmoxele31xu2130iwfjttd25
    foreign key (place_seven_id)
    references place_all;

alter table if exists numbers_statistics_all
    add constraint FKbg1wixuh3441cdhs8yhid9j36
    foreign key (place_six_id)
    references place_all;

alter table if exists numbers_statistics_all
    add constraint FK9lqmu1bodeb1ws253iw9wbduh
    foreign key (place_ten_id)
    references place_all;

alter table if exists numbers_statistics_all
    add constraint FKppc2u90ciopcda9ir108qbkno
    foreign key (place_three_id)
    references place_all;

alter table if exists numbers_statistics_all
    add constraint FK1so0d0hpkip5h489s3t6o4nm5
    foreign key (place_two_id)
    references place_all;

alter table if exists numbers_statistics_dto
    add constraint FKl9yjenx93e54twafl4t6t2j82
    foreign key (place_eight_id)
    references place_dto;

alter table if exists numbers_statistics_dto
    add constraint FK4pwbw9jj00actv6m34ykqwbxe
    foreign key (place_five_id)
    references place_dto;

alter table if exists numbers_statistics_dto
    add constraint FKn2xpg6qg280loyvr2e94bxpeu
    foreign key (place_four_id)
    references place_dto;

alter table if exists numbers_statistics_dto
    add constraint FKaxk6kl6ank7fn4di90tso5xvk
    foreign key (place_nine_id)
    references place_dto;

alter table if exists numbers_statistics_dto
    add constraint FKmd7l396st6t9rkatmgl1jf3p1
    foreign key (place_one_id)
    references place_dto;

alter table if exists numbers_statistics_dto
    add constraint FKa1b21giym5dyof62bwu32nxs1
    foreign key (place_seven_id)
    references place_dto;

alter table if exists numbers_statistics_dto
    add constraint FKneaf95gf9qr1ilaal08orfpdl
    foreign key (place_six_id)
    references place_dto;

alter table if exists numbers_statistics_dto
    add constraint FKkfkf5hdajk42d997p02vy27da
    foreign key (place_ten_id)
    references place_dto;

alter table if exists numbers_statistics_dto
    add constraint FK11qottfw6ndng58xscthmunjm
    foreign key (place_three_id)
    references place_dto;

alter table if exists numbers_statistics_dto
    add constraint FK6bu3x77bhfa5b7ldq6plff4c6
    foreign key (place_two_id)
    references place_dto;

alter table if exists player_dto
    add constraint FK1yofm5ikyjme75rqdbfs41suw
    foreign key (numbers_statistics_id)
    references numbers_statistics_dto;

alter table if exists player_dto
    add constraint FKra3bkhbividrcdyjtqe1idgk7
    foreign key (rating_statistics_id)
    references rating_statistics_dto;

alter table if exists player_dto
    add constraint FK9e06vtuy97neaut5lxo70d0jo
    foreign key (roles_history_statistics_id)
    references roles_history_statistics_dto;

alter table if exists player_dto
    add constraint FKhxyt2e1h4166ed7mf4fp0chx
    foreign key (seriality_statistics_id)
    references seriality_statistics_dto;

alter table if exists player_dto
    add constraint FKm6nbt2oo9maj07url6ar8ujlk
    foreign key (visiting_statistics_id)
    references visiting_statistics_dto;

alter table if exists player_dto_couple_statistics
    add constraint FKk6jy6yy88g7ifxt8t20u8kxhj
    foreign key (couple_statistics_id)
    references couple_statistics_dto;

alter table if exists player_dto_couple_statistics
    add constraint FKgj5oqenyp3cjvs3j05v8mi1al
    foreign key (player_dto_id)
    references player_dto;

alter table if exists player_dto_roles
    add constraint FKmat7yb25vfgdb45657ial5q0e
    foreign key (roles_id)
    references role_dto;

alter table if exists player_dto_roles
    add constraint FKikvnfpmal3ofycuxjluf147ae
    foreign key (player_dto_id)
    references player_dto;
